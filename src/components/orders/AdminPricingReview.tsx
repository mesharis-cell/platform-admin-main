...
export function AdminPricingReview({ orderId, order }: AdminPricingReviewProps) {
  ...
  // Calculate totals
  const catalogTotal = lineItems?.filter((i: any) => i.lineItemType === 'CATALOG' && !i.isVoided)
    .reduce((sum: number, item: any) => sum + item.total, 0) || 0
  const customTotal = lineItems?.filter((i: any) => i.lineItemType === 'CUSTOM' && !i.isVoided)
    .reduce((sum: number, item: any) => sum + item.total, 0) || 0
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <ReskinRequestsList orderId={orderId} orderStatus={order?.orderStatus || order?.order_status} />
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          ...
          {pricing?.base_operations && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <h4 className="text-sm font-semibold text-muted-foreground">BASE OPERATIONS</h4>
                <Badge variant="outline">Auto-calculated</Badge>
              </div>
              <div className="flex justify-between text-sm p-3 bg-muted/30 rounded-md">
                <span>Warehouse Ops ({pricing.base_operations.volume.toFixed(1)} m³ × {pricing.base_operations.rate.toFixed(2)})</span>
                <span className="font-mono font-semibold">{pricing.base_operations.total.toFixed(2)} AED</span>
              </div>
            </div>
          )}
          ...
          {pricing?.transport && (
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm p-3 bg-muted/30 rounded-md">
                <span>
                  {pricing.transport.emirate}, {pricing.transport.trip_type === 'ROUND_TRIP' ? 'Round-trip' : 'One-way'}
                  {' '}({pricing.transport.vehicle_type})
                </span>
                <span className="font-mono font-semibold">{pricing.transport.final_rate.toFixed(2)} AED</span>
              </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-semibold text-muted-foreground">SERVICE LINE ITEMS</h4>
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={() => setAddCatalogOpen(true)}>
          ...
          <div className="space-y-2 bg-blue-50/50 dark:bg-blue-950/20 p-4 rounded-lg">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-mono font-semibold">
                {((pricing?.base_operations?.total || 0) + (pricing?.transport?.final_rate || 0) + catalogTotal).toFixed(2)} AED
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">
                Platform Margin ({marginOverride ? marginPercent : pricing?.margin?.percent || 25}%)
              </span>
              <span className="font-mono">
                {(((pricing?.base_operations?.total || 0) + (pricing?.transport?.final_rate || 0) + catalogTotal) * 
                  ((marginOverride ? marginPercent : pricing?.margin?.percent || 25) / 100)).toFixed(2)} AED
              </span>
            </div>
            {customTotal > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Custom Charges (no margin)</span>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-xl font-bold font-mono text-primary">
                {(((pricing?.base_operations?.total || 0) + (pricing?.transport?.final_rate || 0) + catalogTotal) * 
                  (1 + ((marginOverride ? marginPercent : pricing?.margin?.percent || 25) / 100)) + customTotal).toFixed(2)} AED
              </span>
            </div>
          </div>
          ...
          <div className="space-y-3 border-t border-border pt-4">
            ...
            {marginOverride && (
              <div className="space-y-3 pl-6 border-l-2 border-primary">
                <div>
                  <Label>Margin Percent (%) <span className="text-destructive">*</span></Label>
                  <Input
                    type="number"
                    step="0.1"
                    min="0"
          ...
          <div className="flex gap-3 pt-4">
            <Button
              ...
              size="lg"
            >
              {adminApproveQuote.isPending ? 'Approving...' : 'Approve & Send Quote to Client'}
            </Button>
            <Button
              variant="outline"
