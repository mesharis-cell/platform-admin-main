        ...
        <div className="space-y-4">
          <div className="p-3 bg-muted rounded-md space-y-1 text-sm">
            ...
            <div>
              ...
              <span className="font-mono font-semibold">{costAmount.toFixed(2)} AED</span>
            </div>
          </div>
          ...
          <div>
            ...
            <RadioGroup value={orderAction} onValueChange={(v: any) => setOrderAction(v)}>
              <div className="flex items-start space-x-2 p-3 border border-border rounded-md">
                ...
                <Label htmlFor="continue" className="flex-1 cursor-pointer">
                  <div>
                    <p className="font-semibold">Continue with original asset</p>
                    <p className="text-sm text-muted-foreground">
                      Order will proceed with "{originalAssetName}". Quote will be revised and client must acknowledge.
                    </p>
                  </div>
                </Label>
              </div>
              ...
              <div className="flex items-start space-x-2 p-3 border border-border rounded-md">
                <RadioGroupItem value="cancel_order" id="cancel_order" className="mt-1" />
          ...
          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded-md p-3">
            <p className="text-xs text-amber-800 dark:text-amber-300">
              The {costAmount.toFixed(2)} AED line item will be voided and removed from the quote.
            </p>
          </div>
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={cancelReskin.isPending}>
            Back
          </Button>
          <Button onClick={handleCancel} variant="destructive" disabled={cancelReskin.isPending}>
