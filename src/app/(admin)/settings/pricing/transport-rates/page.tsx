...
const EMIRATES = ['Dubai', 'Abu Dhabi', 'Al Ain', 'Sharjah', 'Ajman', 'Ras Al Khaimah', 'Umm Al Quwain', 'Fujairah']
...
export default function TransportRatesPage() {
  ...
  const handleDelete = async (rate: TransportRate) => {
    if (!confirm(`Deactivate transport rate for ${rate.emirate} (${rate.tripType}, ${rate.vehicleType})?`)) {
      return
    }
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <p className="text-muted-foreground text-center py-8">No transport rates configured</p>
            ) : (
                  ...
                  >
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        <span className="font-semibold">{rate.emirate}</span>
                        <Badge variant="outline">{rate.tripType}</Badge>
                        <Badge variant="outline">{rate.vehicleType}</Badge>
                        {!rate.isActive && <Badge variant="destructive">Inactive</Badge>}
                      </div>
                      {rate.companyId && (
                        <p className="text-xs text-muted-foreground mt-1">Company-specific override</p>
                      )}
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="text-lg font-bold font-mono">{rate.rate.toFixed(2)} AED</span>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              <Label>Emirate <span className="text-destructive">*</span></Label>
            ...
            <div>
              ...
              <Select value={formData.tripType} onValueChange={(v: any) => setFormData({ ...formData, tripType: v })}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Select value={formData.vehicleType} onValueChange={(v: any) => setFormData({ ...formData, vehicleType: v })}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              <Label>Rate (AED) <span className="text-destructive">*</span></Label>
              <Input
                type="number"
                step="0.01"
                min="0"
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="p-3 bg-muted rounded-md text-sm">
              <p><strong>Emirate:</strong> {selectedRate?.emirate}</p>
              <p><strong>Trip Type:</strong> {selectedRate?.tripType}</p>
              <p><strong>Vehicle:</strong> {selectedRate?.vehicleType}</p>
            </div>
            <div>
              <Label>Rate (AED) <span className="text-destructive">*</span></Label>
              <Input
