  ...
  return (
    <div className="min-h-screen bg-background">
      <AdminHeader
        ...
        stats={data?.data ? { label: 'AWAITING APPROVAL', value: data.data.length } : undefined}
        actions={
          <Link href="/orders">
                      ...
                      <div className="flex gap-2">
                        ...
                        {hasReskins && (
                          <Badge variant="outline" className="bg-amber-50 dark:bg-amber-950/20">
                            {reskinCount} Rebrand{reskinCount > 1 ? 's' : ''}
                          </Badge>
                        )}
                      </div>
                    </div>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{order.venue_location?.city}</p>
                      </div>
                      <div>
                    ...
                    {hasReskins && (
                      <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded-md p-3">
                        <p className="text-xs text-amber-800 dark:text-amber-300">
                          🔄 This order includes {reskinCount} rebrand request{reskinCount > 1 ? 's' : ''} that need processing
                        </p>
                      </div>
                    )}
                    ...
                    {order.pricing && (
                      <div className="text-sm">
                        ...
                        <p className="text-xl font-bold font-mono">
                          {order.pricing.final_total?.toFixed(2) || 'TBD'} AED
                        </p>
                      </div>
                    )}
