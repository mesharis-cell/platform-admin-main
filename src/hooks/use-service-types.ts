...
import type { ServiceType, CreateServiceTypeRequest, UpdateServiceTypeRequest } from '@/types/hybrid-pricing'
...
export function useListServiceTypes(filters: Record<string, any> = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiClient.get(`/client/v1/pricing/service-types?${queryParams}`)
...
export function useUpdateServiceType() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: UpdateServiceTypeRequest }) => {
      try {
        const response = await apiClient.put(`/client/v1/pricing/service-types/${id}`, data)
        return response.data.data
      } catch (error) {
        throwApiError(error)
      }
