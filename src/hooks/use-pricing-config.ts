...
export function useSetCompanyOverride() {
  ...
  return useMutation({
    mutationFn: async ({ companyId, data }: { companyId: string; data: SetPricingConfigRequest }) => {
      try {
        const response = await apiClient.put(`/client/v1/pricing/config/${companyId}`, data)
        return response.data.data
      } catch (error) {
        throwApiError(error)
      }
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: pricingConfigKeys.company(variables.companyId) })
    },
  })
}
