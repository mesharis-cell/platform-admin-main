...
export function useProcessReskinRequest(orderId: string) {
  ...
  return useMutation({
    mutationFn: async ({ orderItemId, data }: { orderItemId: string; data: ProcessReskinRequestRequest }) => {
      try {
...
export function useCompleteReskinRequest() {
  ...
  return useMutation({
    mutationFn: async ({ reskinId, data }: { reskinId: string; data: CompleteReskinRequestRequest }) => {
      try {
        const response = await apiClient.post(`/client/v1/reskin-requests/${reskinId}/complete`, data)
        return response.data.data
      } catch (error) {
        throwApiError(error)
      }
...
export function useCancelReskinRequest() {
  ...
  return useMutation({
    mutationFn: async ({ reskinId, data }: { reskinId: string; data: CancelReskinRequestRequest }) => {
      try {
        const response = await apiClient.post(`/client/v1/reskin-requests/${reskinId}/cancel`, data)
        return response.data.data
      } catch (error) {
        throwApiError(error)
      }
