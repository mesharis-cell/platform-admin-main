...
export function useCreateCatalogLineItem(orderId: string) {
  ...
  return useMutation({
    mutationFn: async (data: CreateCatalogLineItemRequest) => {
      try {
        const response = await apiClient.post(`/client/v1/order/${orderId}/line-items/catalog`, data)
        return response.data.data
      } catch (error) {
        throwApiError(error)
      }
...
export function useCreateCustomLineItem(orderId: string) {
  ...
  return useMutation({
    mutationFn: async (data: CreateCustomLineItemRequest) => {
      try {
        const response = await apiClient.post(`/client/v1/order/${orderId}/line-items/custom`, data)
        return response.data.data
      } catch (error) {
        throwApiError(error)
      }
...
export function useUpdateLineItem(orderId: string) {
  ...
  return useMutation({
    mutationFn: async ({ itemId, data }: { itemId: string; data: UpdateLineItemRequest }) => {
      try {
        const response = await apiClient.put(`/client/v1/order/${orderId}/line-items/${itemId}`, data)
        return response.data.data
      } catch (error) {
        throwApiError(error)
      }
...
export function useVoidLineItem(orderId: string) {
  ...
  return useMutation({
    mutationFn: async ({ itemId, data }: { itemId: string; data: VoidLineItemRequest }) => {
      try {
        const response = await apiClient.delete(`/client/v1/order/${orderId}/line-items/${itemId}`, {
          data,
        })
        return response.data.data
      } catch (error) {
