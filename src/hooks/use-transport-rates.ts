...
import type { TransportRate, CreateTransportRateRequest, UpdateTransportRateRequest } from '@/types/hybrid-pricing'
...
export function useListTransportRates(filters: Record<string, any> = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiClient.get(`/client/v1/pricing/transport-rates?${queryParams}`)
...
export function useUpdateTransportRate() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: UpdateTransportRateRequest }) => {
      try {
        const response = await apiClient.put(`/client/v1/pricing/transport-rates/${id}`, data)
        return response.data.data
      } catch (error) {
        throwApiError(error)
      }
  ...
  return useMutation({
    mutationFn: async (params: { emirate: string; tripType: string; vehicleType: string }) => {
      try {
        ...
        const response = await apiClient.get(`/client/v1/pricing/transport-rate/lookup?${queryParams}`)
        return response.data.data
      } catch (error) {
        throwApiError(error)
      }
